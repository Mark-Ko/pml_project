...
####Recursive Regression Tree  
  
For my first model I used recursive regression tree partitioning with "rpart" in caret. I used 7- fold cross validation. The final model consisted of four features. The accuracy of the model on the test set was 0.49, roughly the same as random guessing.  
...
ptree <- predict(modtree, newdata = testing)
confusionMatrix(ptree, testing$classe)$overall[1:2]
```
  
####Naive Bayes  
  
I next tried a naive Bayes model with "nb" in caret. Again I used 7-fold cross validation. The accuracy on the test set was 0.7395 (out of sample error of 26.05%), better than my regression tree model, but still not accurate enough.  
...
For my last model decided to try a random forest. During "play" I found that using the function randomForest() was much faster than using the caret train() wrapper.  [Random forest doesn't require cross-validation]( http://www.stat.berkeley.edu/~breiman/RandomForests/cc_home.htm#ooberr). The out-of-bag (OOB) error is an unbiased estimate of the classification error as trees are added to the forest. I decided to cross-validate with the testing set out of fear (irrational perhaps!) of over-fitting. The OOB classification error of my model was 0.53%. The out of sample error was 0.42%; showing that cross-validation really wasn't needed.   
  
I list the 20 most important features in my "black box" random forest model and plot them in Figure 2. It's a little surprising that three of the top five features were sensors on the belt, not on the forearm or dumbbell. The error rates for the different classes, reached their minimums after approximately 100 trees, Figure 3. The smallest errors were for the ideal biceps curl and the curl with hips forward (Class E). An overlaid density plot for the top 8 features shows that there doesn't appear to be simple separability of classes, except perhaps for pitch forearm and roll forearm, Figure 4. 
...
I was able to predict the "quality" of a dumbbell curl. My random forest model was surprisingly accurate, 99.58% on the cross-validation testing set. Recursive regression tree partitioning was disappointing as was naive Bayes. For the top features, the probability density functions of the classes were similar and didn't appear easily separable. This may have contributed to the poor performance of those two methods. Based  on the biomechanics of the system, I would expect some sensor variables to be coupled, thus dependent, a violation of the "naive" assumption. This may explain the performance of the naive Bayes classifier.  
